...
//serverCreate[listenSocketId, clientLengthMax, bufferSize]: serverPtr
...
#pragma region server select ready sockets

//serverSelectReadySockets[socketId]: successState
DLLEXPORT int socketClose(WolframLibraryData libData, mint Argc, MArgument *Args, MArgument Res){
    SOCKET socketId = MArgument_getInteger(Args[0]); // positive integer
...
DLLEXPORT int socketSelect(WolframLibraryData libData, mint Argc, MArgument *Args, MArgument Res){
    ...
    for (size_t i = 0; i++; i <result){
        libData->MTensor_setInteger(readySockets, &i, )
    }
...
//serverAddClient[server, client]: void
...
DLLEXPORT int serverAccept(WolframLibraryData libData, mint Argc, MArgument *Args, MArgument Res){
    Server server = (Server)MArgument_getInteger(Args[0]);
    ...
    if (client == INVALID_SOCKET){
        ...
        #ifdef _DEBUG
        printf("%s[serverAccept->ERROR]%s\n\taccept(listenSocket = %I64d) returns error = %d\n\n", 
            RED, RESET, server->listenSocket, GETSOCKETERRNO());
        #endif
...
DLLEXPORT int serverGetListenSocket(WolframLibraryData libData, mint Argc, MArgument *Args, MArgument Res){
    ...
    #ifdef _DEBUG
    printf("%s[serverGetListenSocket->SUCCESS]%s\n\tlistenSocket = %I64d\n\n", 
        GREEN, RESET, server->listenSocket);
    #endif
...
bool clientsReadyQ(Server server, fd_set *read_set) {
    ...
    SOCKET maxSocket = server->maxClientId;
